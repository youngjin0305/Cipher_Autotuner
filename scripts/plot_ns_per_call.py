#!/usr/bin/env python3
"""Plot corrected trimmed mean ns/call versus input length."""
...
def plot_for_key_bits(rows: list[dict[str, str]], output_dir, key_bits: int) -> None:
    """Render one ns/call landscape figure for a single key size."""
    metric_column, metric_label, metric_stem = resolve_metric_column(rows, "ns_call")
    ...
    ax.legend(loc="upper left", **legend_style_kwargs())
...
def main() -> None:
    parser = build_metric_parser(
        "Plot corrected trimmed mean ns/call by input length for the main ARIA implementations.",
        default_metric="ns_call",
    )
